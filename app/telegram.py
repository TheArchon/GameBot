...
import json, time, urllib.parse, urllib.request, mimetypes, uuid
...
class Telegram:
    ...
    def call(self,method:str,**params):
        ...
        encoded=urllib.parse.urlencode({k: json.dumps(v,ensure_ascii=False) if isinstance(v,(dict,list)) else str(v) for k,v in data.items()}).encode()
        try:
        ...
        except Exception as e: raise TelegramError(str(e)) from e
        if not payload.get("ok"): raise TelegramError(payload.get("description","Telegram API error"))
        return payload.get("result")
    def get_updates(self,offset:int|None,timeout:int=25): return self.call("getUpdates",offset=offset,timeout=timeout,allowed_updates=["message","callback_query"])
    def send(self,chat_id,text,reply_markup=None,reply_to=None,parse_mode="HTML"):
        if parse_mode == "HTML": text = md_to_html(str(text))
    ...
    def edit_caption(self,chat_id,message_id,caption,reply_markup=None,parse_mode="HTML"):
        if parse_mode == "HTML": caption = md_to_html(str(caption))
    ...
    def answer_callback(self,cid,text="",alert=False): return self.call("answerCallbackQuery",callback_query_id=cid,text=text,show_alert=alert)
    ...
    def send_photo(self, chat_id, photo_path, caption="", reply_markup=None, reply_to=None, parse_mode="HTML"):
        ...
        if reply_markup is not None: field("reply_markup", json.dumps(reply_markup, ensure_ascii=False))
        if reply_to is not None: field("reply_to_message_id", reply_to)
        ...
        body=b"".join(parts)
        req=urllib.request.Request(f"{self.base}/sendPhoto",data=body,headers={"Content-Type":f"multipart/form-data; boundary={boundary.decode()}"},method="POST")
        try:
            with urllib.request.urlopen(req,timeout=45) as r: payload=json.loads(r.read().decode())
        except Exception as e: raise TelegramError(str(e)) from e
        if not payload.get("ok"): raise TelegramError(payload.get("description","Telegram API error"))
    ...
    def get_me(self): return self.call("getMe")
